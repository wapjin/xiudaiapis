...
from Model.getdata import get_orderid
import time
# 添加我的采购单信息
# 已测试
class add_myorder(BaseModel):

    dingding_name: str=""
    dingding_bumen: str=""
    dingding_xm: str
    dingding_endtime: str=""
    dingding_type: str=""
    wuliao_lei: str=""
    xiadan_time: str=""
    canpin_name: str=""
...
@app.post('/addmyorder')
async def up_myorders(*, Authorization: str = Header(None), data: add_myorder):
    # try:
    ...
    if msg == 1:
        if data.dingding_xm:
            ...
            field_list=["order_id","dingding_name",
            "dingding_bumen",
            "dingding_xm",
            "dingding_endtime",
            "dingding_type",
            ...
            value_list.append(data.dingding_name)
            value_list.append(data.dingding_bumen)
            value_list.append(data.dingding_xm)
            value_list.append(data.dingding_endtime)
            value_list.append(data.dingding_type)
            value_list.append(data.wuliao_lei)
            value_list.append(data.xiadan_time)
